...
// orders generator wrote, preserving sum(o_ol_cnt) == count(order_line).
var orderLineTable = &table{
	...
	nStreams: 3, // ol_i_id, ol_amount, ol_dist_info
...
func genOrderLine(w *world, b *mem.RowBuf, cycle int64, s []rng.Stream) {
	...
	for ln := int64(1); ln <= olCnt; ln++ {
		...
		b.AppendInt64(2, wID)
		b.AppendInt64(3, ln)
		b.AppendInt64(4, rng.UniformInt(s[0], lc, 1, itemsCount)) // ol_i_id
		...
		if delivered {
			...
			b.AppendFloat64(8, rf(s[1], lc, 0.01, 9999.99))
		}
		aStrFixed(dist[:], s[2], lc)
