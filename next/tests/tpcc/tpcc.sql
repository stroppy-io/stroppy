...
--= count_low_stock
-- One-shot stock_level count (next engine supplement): the last-20-orders item
-- window joined to stock, counting distinct low-stock items. Equivalent to v5's
-- two-step get_window_items + stock_count_in (and the SLEV proc body), collapsed
-- to a single prepared statement so no IN-list is built or interpolated on the
-- hot path. The Args API binds :w_id/:d_id/:min_o_id/:next_o_id/:threshold directly.
