...
async function buildProtobufSDK() {
    ...
    const stroppyFiles = collectTsFiles(path.join(tsSourceDir, 'proto', 'stroppy'));
    const googleFiles = collectTsFiles(path.join(tsSourceDir, 'google', 'protobuf'));

    // Create entry file that re-exports everything
    const entryPath = path.join(__dirname, '_entry.ts');
    const entryContent = stroppyFiles
        .map(file => `export * from './${path.relative(__dirname, file).replace(/\\/g, '/').replace(/\.ts$/, '')}';`)
        .join('\n');
    ...
    const combinedTS = [
        ...
        ...[...googleFiles, ...stroppyFiles].map(file => {
...
async function buildAnalyzeDDL() {
    ...
    await esbuild.build({
        entryPoints: [path.join(__dirname, 'analyze_ddl_entry.ts')],
        bundle: true,
        format: 'esm',
        platform: 'node',
        target: 'es2020',
