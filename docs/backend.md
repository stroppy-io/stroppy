...
│   └── middleware/             # Middleware для аутентификации и CORS
└── pkg/                        # Переиспользуемые пакеты
    ├── auth/                   # JWT аутентификация
    ├── database/               # Работа с базой данных и конфиги
    └── proto/                  # Сгенерированные protobuf типы
```
...
- `make run` / `make dev` — запустить локально
- `make test`, `make test-coverage` — тесты и покрытие
- `make docker-up` / `make docker-dev` — запустить docker-compose в prod/dev режимах
- `make docker-clean` — остановить и очистить Docker-ресурсы
- `make proto` — пересобрать protobuf артефакты
- `make fmt`, `make lint`, `make clean` — сервисные задачи
