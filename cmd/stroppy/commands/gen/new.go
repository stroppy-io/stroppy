...
var availablePresetsStr = strings.Join(workloads.AvailablePresets(), ", ")
...
`, availablePresetsStr),
	...
	_ = cmd.Flags().
		String(workdirFlagName, "", "output directory for development environment")

	_ = cmd.Flags().String(presetFlagName, "",
		fmt.Sprintf("preset example to include (%s)", availablePresetsStr),
	)

	if err := cmd.MarkFlagRequired(workdirFlagName); err != nil {
		panic(err)
	}

	if err := cmd.MarkFlagDirname(workdirFlagName); err != nil {
		panic(err)
	}
