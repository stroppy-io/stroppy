-- TPC-DS query workload for MySQL 8.0. 99 queries (14/23/24/39 are two-part:
-- _a/_b), generated from the official templates at the canonical qualification
-- parameters (RNGSEED 19620718, SCALE 1). MySQL dialect transforms vs ANSI:
-- date interval arithmetic, GROUP BY ... WITH ROLLUP, CONCAT for pipe concat,
-- the q30 c_last_review_date column-name fix, lochierarchy inlined into ORDER BY,
-- and a div-by-zero guard on q90. Each query is one named section, per parse_sql.
...
select  a.ca_state state, count(*) cnt
 ...
 where       a.ca_address_sk = c.c_current_addr_sk
 	...
 	and i.i_category = icat.i_category
 	and d.d_month_seq = 
...
with  cross_items as
 ...
 where ws_item_sk = iws.i_item_sk
   ...
   and d3.d_year between 1999 AND 1999 + 2) dt1
 ...
 from (select cs_quantity*cs_list_price sales
      ...
      select ws_quantity*ws_list_price sales
       ...
       where d_year = 2000 
         ...
         and ws_bill_customer_sk in (select c_customer_sk from best_ss_customer)) dt2 
 limit 100
;
...
with frequent_ss_items as
 ...
 from (select c_last_name,c_first_name,sum(cs_quantity*cs_list_price) sales
      ...
      select c_last_name,c_first_name,sum(ws_quantity*ws_list_price) sales
       ...
       group by c_last_name,c_first_name) dt2 
...
from (select item_sk
     ...
     from (select case when web.item_sk is not null then web.item_sk else store.item_sk end item_sk
           ...
           select case when web.item_sk is not null then web.item_sk else store.item_sk end item_sk
                 ...
                 ,web.cume_sales web_sales
                 ,store.cume_sales store_sales
           from web_v1 web right join store_v1 store on (web.item_sk = store.item_sk and web.d_date = store.d_date)
...
group by cs_bill_customer_sk
        ,cs_item_sk)
 select  sum(case when t.scs is not null and t.ccs is null then 1 else 0 end) store_only
      ,sum(case when t.scs is null and t.ccs is not null then 1 else 0 end) catalog_only
      ,sum(case when t.scs is not null and t.ccs is not null then 1 else 0 end) store_and_catalog
from (
  select ssci.customer_sk scs, csci.customer_sk ccs
  from ssci left join csci on (ssci.customer_sk=csci.customer_sk and ssci.item_sk = csci.item_sk)
  union all
  select ssci.customer_sk scs, csci.customer_sk ccs
  from ssci right join csci on (ssci.customer_sk=csci.customer_sk and ssci.item_sk = csci.item_sk)
