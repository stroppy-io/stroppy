-- TPC-DS query suite for picodata (sbroad). Derived from pg.sql by the
-- porting script; hand rewrites for the sbroad gaps land here on top of the
-- mechanical base.
--
-- *** STATUS: query EXECUTION is NOT yet supported on picodata. ***
-- Only the data-load path ships today (schema.pico.sql + load). The query
-- bodies below are the mechanical-transform skeleton (date/char/cast fixes,
-- date+N baked); they DO NOT EXECUTE on sbroad yet. The blocking gap:
--
--   sbroad (picodata 26.3) has no comma-join and no CROSS JOIN.
--   FROM a, b  -> "expected IndexedByExpr"; CROSS JOIN -> "expected Identifier".
--   No PRAGMA / session setting enables implicit cross joins (YDB has
--   AnsiImplicitCrossJoin; sbroad does not). Only INNER JOIN ... ON works.
--
-- ~88 of 95 TPC-DS queries use comma joins (TPC-DS template style), so they
-- fail at parse time. Secondary gaps behind that one: rollup/grouping sets,
-- INTERSECT, FULL OUTER JOIN, rank()/dense_rank()/lag()/lead(), correlated
-- subqueries, datetime+int, round(). See the PR/issue tracking the pico query
-- port for the plan (AST comma-join rewrite + the targeted secondary rewrites;
-- or an upstream sbroad implicit-cross-join feature).
--
-- This file is kept as the port starting point. To load picodata today, run
-- with `--no-steps workload` (drop/create/load/index only); the workload step
-- is expected to fail until the query port lands.
--
-- sbroad gaps recorded for the future port (probed against picodata 26.3):
--   - char/date types absent -> schema.pico.sql uses varchar/datetime.
--   - no datetime+int / DATE type -> date+N offsets baked to literals below.
--   - rank()/dense_rank()/lag()/lead(): no engine support, no correlated
--     subquery to emulate. 8 queries are SKIPPED here and logged at startup:
--     query_36, query_44, query_47, query_49, query_57, query_67,
--     query_70, query_86.
--   - rollup/grouping sets, INTERSECT, FULL OUTER JOIN, correlated
--     subqueries to be rewritten where the queries that need them appear.
--
...
with ssr as
 ...
 where date_sk = d_date_sk
       and d_date between cast('2000-08-19' as datetime) 
                  and (cast('2000-09-02' as datetime))
...
select  a.ca_state state, count(*) cnt
 from customer_address a
     ...
     ,date_dim d
     ,item i
     ,(select i_category as iavg_cat, avg(i_current_price) as iavg_price from item group by i_category) iavg
...
where 
	ws_item_sk = i_item_sk 
  	...
  	and ws_sold_date_sk = d_date_sk
	and d_date between cast('2001-06-15' as datetime) 
				and (cast('2001-07-15' as datetime))
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
select  i_item_id
 ...
 where cs_item_sk = i_item_sk 
   ...
   and cs_sold_date_sk = d_date_sk
 and d_date between cast('2002-06-18' as datetime) 
 				and (cast('2002-07-18' as datetime))
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
...
select  *
 from(select w_warehouse_name
            ,i_item_id
            ,sum(case when (cast(d_date as datetime) < cast ('1999-06-22' as datetime))
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and d_date between (cast('1999-05-23' as datetime))
                    and (cast('1999-07-22' as datetime))
...
select  i_item_id
 ...
 and d_date between cast('2002-03-29' as datetime) and (cast('2002-05-28' as datetime))
...
select  
   w_state
  ,i_item_id
  ,sum(case when (cast(d_date as datetime) < cast ('2001-05-02' as datetime)) 
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast('2001-04-02' as datetime))
                and (cast('2001-06-01' as datetime)) 
 group by
    w_state,i_item_id
...
with ss as
 ...
 from store_sales,
      ...
      store
 where ss_sold_date_sk = d_date_sk
       and d_date between cast('2000-08-10' as datetime) 
                  and (cast('2000-09-09' as datetime)) 
       and ss_store_sk = s_store_sk
 group by s_store_sk)
 ,
 sr as
...
ss as
 ...
 select 
ss_customer_sk,
round(ss_qty::numeric/(coalesce(ws_qty,0)+coalesce(cs_qty,0)),2) ratio,
...
with ssr as
 (select  s_store_id as store_id,
  ...
  from store_sales left outer join store_returns on
     ...
     promotion
 where ss_sold_date_sk = d_date_sk
       and d_date between cast('2002-08-14' as datetime) 
                  and (cast('2002-09-13' as datetime))
       and ss_store_sk = s_store_sk
       and ss_item_sk = i_item_sk
...
select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast('2001-01-13' as datetime) and (cast('2001-03-14' as datetime))
...
where 
	ss_item_sk = i_item_sk 
  	...
  	and ss_sold_date_sk = d_date_sk
	and d_date between cast('1999-02-05' as datetime) 
				and (cast('1999-03-07' as datetime))
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
