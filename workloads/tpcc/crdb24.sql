...
BEGIN
  ...
  WHILE loop_counter < no_o_ol_cnt LOOP
    loop_counter := loop_counter + 1;
    v_i_id := 1 + (floor(random() * 100000.0))::INTEGER;
    ...
    IF no_max_w_id > no_min_w_id AND floor(random() * 100.0)::INTEGER = 0 THEN
      v_supply_w_id := no_min_w_id + (floor(random() * (no_max_w_id - no_min_w_id)::DOUBLE PRECISION))::INTEGER;
...
BEGIN
  ...
  IF ( byname = 1 ) THEN
    ...
    IF name_count > 0 THEN
      ...
      LIMIT 1 OFFSET GREATEST(0, floor((name_count - 1) / 2.0))::INTEGER;
    END IF;
  ELSE
  ...
  SET c_balance = c_balance - p_h_amount,
      ...
      c_data = CASE
        WHEN c_credit = 'BC' THEN SUBSTR(
          ...
          ' ' || CAST(CAST(p_h_amount AS DECIMAL(12,2)) AS STRING) || '|' || COALESCE(c_data, ''),
          1, 500
        )
        ELSE c_data
      END
...
BEGIN
  ...
  IF ( byname = 1 ) THEN
    ...
    IF namecnt > 0 THEN
      ...
      LIMIT 1 OFFSET GREATEST(0, floor((namecnt - 1) / 2.0))::INTEGER;
    END IF;
  ELSE
