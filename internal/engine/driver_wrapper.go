...
//   - vu.State() == nil (init phase): shared driver, dispatched lazily via root slot
...
func (d *DriverWrapper) ensureReady() {
	d.readyOnce.Do(func() {
		...
		if d.shared {
			d.drv = root.initSharedDriver(d.driverIndex, d.vu, d.cfg)
		} else {
			d.lg = d.lg.With(zap.Uint64("VUID", d.vu.VUID()))
			var err error
...
func (d *DriverWrapper) RunQuery(sql string, args map[string]any) (*driver.QueryResult, error) {
	...
	root.txMetrics.recordQuery(d.vu)
...
func (d *DriverWrapper) InsertSpecBin(specBin []byte) (*stats.Query, error) {
	...
	root.txMetrics.recordInsert(d.vu, spec.GetTable(), result.Rows)
...
func (d *DriverWrapper) InsertTpch(table string, scaleFactor float64, workers int) (*stats.Query, error) {
	...
	root.txMetrics.recordInsert(d.vu, table, result.Rows)
...
func (d *DriverWrapper) newInsertProgressTracker(
	...
	config.OnSample = func(snapshot insertprogress.Snapshot) {
		root.txMetrics.recordInsertProgress(d.vu, snapshot)
	}
...
func (t *TxWrapper) RunQuery(sql string, args map[string]any) (*driver.QueryResult, error) {
	...
	root.txMetrics.recordQuery(t.vu)
...
func (t *TxWrapper) Commit() error {
	...
	root.txMetrics.record(t.vu, "commit", t.name, t.isolation)
	return nil
}
...
func (t *TxWrapper) Rollback() error {
	...
	root.txMetrics.record(t.vu, "rollback", t.name, t.isolation)
	return nil
}
