...
func TestProcessArgs(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "duplicate arguments (dedup)",
		...
		},
		{
			...
			wantArgs: nil,
			wantErr:  fmt.Errorf("%w: [user_id]", sqldriver.ErrExtraArgument),
		},
		{
			name:     "comma test",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gotSQL, gotArgs, gotErr := sqldriver.ProcessArgs(pgxDialect, tt.sql, tt.args)
