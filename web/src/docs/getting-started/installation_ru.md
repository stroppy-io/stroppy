...
```bash
# Установка Node.js (для веб-интерфейса)
...
После установки откройте браузер и перейдите по адресу `http://localhost:5173` (режим разработки) или `http://localhost:8080` при запуске через Docker Compose.
