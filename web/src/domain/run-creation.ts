...
export interface WorkloadProperties {
  runners: number // количество параллельных запусков нагрузки
  threads?: number
  duration: number // длительность теста в минутах
  
  // Свойства для конкретных типов нагрузок
  ...
  tpcdsProperties?: TPCDSProperties
...
export interface TPCDSProperties {
  scaleFactor: number
  queries?: number
}

// Утилитарные типы и функции
export const WORKLOAD_TYPES: WorkloadTypeOption[] = [
  {
    value: 'pgbench',
    label: 'pgbench',
    description: 'Бенчмарк PostgreSQL pgbench'
  },
  {
    value: 'ycsb',
    label: 'YCSB',
